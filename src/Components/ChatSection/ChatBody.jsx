          ...
          >
          {
            ...
            msgObj.path.map(file=>{
              ...
              const lastName = splitedName[splitedName.length-1] 
                ...
                {commonImageFormats.includes(extension)?<img src={fileUrl} className='w-[100%] max-h-[350px]' alt="not found" />:commonDocumentFormats.includes(extension)? <iframe className='max-h-[350px]'
  src={`${fileUrl}`}  
  width="100%" 
  height="400px"
  ></iframe>:<div><p className={`text-[${isSelf?'#dedee0':'#503dfc'}] p-2 ${isSelf?'bg-[#2b75ed]':'bg-[#d7d7d9]'} mb-4`}>{lastName.length>25?lastName.substring(0,25)+'...':lastName}</p><a href={fileUrl}  download={lastName}><Button variant='contained' color='secondary' style={{fontSize:'0.8em'}}>Download to see</Button></a></div>
